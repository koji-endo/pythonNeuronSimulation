...
for tfreq,sfreq,bness,dtion in product(timefreqlist,spatialfreqlist,boldness_list,direction_list):
    ...
    stim_duration = 5000
    stim_power = 1.0
    filename = "tf" + str(timefreq)+ "sf"+ str(spatialfreq)+ "direct"+str(stim_direction)
    # movie setting
    ...
    if NOCONVERT is False:
        ...
        for x,y in product(range(width),range(height)):
            ...
            print("{},{},{}".format(start,end,period))
            for t in rrange(0, stim_offset + stim_duration, period*1000):
                stim = {}
                stim["stimulator"] = "BellCurrent"
                stim["target_cellname"] = "R1-6" + "," + str(x) + "," + str(y)
                stim["section"] = {"name":"axon","point":0.5}
                stim["opt"] = {}
                stim["opt"]["st"] = start*1000 + t
                stim["opt"]["en"] = end*1000 + t
                stim["opt"]["amp"] = 1.0
                stim["opt"]["slope"] = 0.03
                stims.append(stim)

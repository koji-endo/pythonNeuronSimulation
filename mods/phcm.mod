...
FUNCTION y2inf(v (mV)) {
  y2inf = pow(1/(1+exp((-23.7-v)/12.8)), 1/3)
}
FUNCTION y2tau(v (mV)) {
  y2tau = 0.13 + 3.39 * exp(-pow((-73-v)/20 ,2))
}
FUNCTION y3inf(v (mV)) {
  y3inf = 0.9 / (1 + exp((-55-v)/(-3.9))) + 0.1 / (1 + exp((-74.8 - v)/(-10.7)))
}
FUNCTION y3tau(v (mV)) {
  y3tau = 113 * exp(-pow((-71-v)/29 ,2))
}
FUNCTION y4inf(v (mV)) {
  y4inf = pow(1/(1+exp((-1-v)/9.1)), 1/2)
}
FUNCTION y4tau(v (mV)) {
  y4tau = 0.5 + 5.75 * exp(-pow((-25-v)/32 ,2))
}
FUNCTION y5inf(v (mV)) {

...
SIMFILE="${EXECPATH}/testdata/opticlobe/10by10/run0.json"

cd $EXECPATH
nrnivmodl mods/
mpirun python $PYTHONPROGRAM -s $SIMFILE

SIMFILE="${EXECPATH}/testdata/opticlobe/10by10/run90.json"
mpirun python $PYTHONPROGRAM -s $SIMFILE
SIMFILE="${EXECPATH}/testdata/opticlobe/10by10/run180.json"
mpirun python $PYTHONPROGRAM -s $SIMFILE
SIMFILE="${EXECPATH}/testdata/opticlobe/10by10/run270.json"
mpirun python $PYTHONPROGRAM -s $SIMFILE
